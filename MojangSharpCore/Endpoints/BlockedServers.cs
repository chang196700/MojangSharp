...
using static MojangSharpCore.Responses.BlockedServersResponse;
...
namespace MojangSharpCore.Endpoints
{
    ...
    public class BlockedServers : IEndpoint<BlockedServersResponse>
    {
        ...
        /// </summary>
        public BlockedServers() => Address = new Uri($"https://sessionserver.mojang.com/blockedservers");

        /// <summary>
        /// Performs a Block Server request
        /// </summary>
        /// <returns></returns>
        public override async Task<BlockedServersResponse> PerformRequestAsync()
        {
            Response = await Requester.Get(this);

            if (Response.IsSuccess)
            {
                return new BlockedServersResponse(Response)
                {
                    BlockedServers = BlockedServer.Parse(Response.RawMessage)
                };
            }
            else
            {
                return new BlockedServersResponse(Error.GetError(Response));
            }
        }
    }
}
